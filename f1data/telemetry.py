...
def get_fastest_telemetry(session, driver):
    ...
    return tel[["Distance", "Speed", "SessionTime"]]

def align_telemetry(tel1, tel2):
    if tel1 is None or tel2 is None or tel1.empty or tel2.empty:
        return None

    tel1 = tel1[["Distance", "Speed"]]
    tel2 = tel2[["Distance", "Speed"]]

    min_dist = max(tel1["Distance"].min(), tel2["Distance"].min())
    max_dist = min(tel1["Distance"].max(), tel2["Distance"].max())
    common_dist = np.linspace(min_dist, max_dist, 500)  # 500 evenly spaced points

    tel1_interp = tel1.set_index("Distance").reindex(common_dist, method='nearest').interpolate("index")
    tel2_interp = tel2.set_index("Distance").reindex(common_dist, method='nearest').interpolate("index")
